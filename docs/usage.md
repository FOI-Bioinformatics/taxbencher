...
Taxbencher is designed to evaluate taxonomic classification tools using standardized metrics. The pipeline accepts taxonomic profiles in taxpasta format (typically generated by [nf-core/taxprofiler](https://nf-co.re/taxprofiler)) and compares them against a gold standard using CAMI OPAL evaluation framework.

### Workflow overview

1. **Input**: Taxonomic profiles from multiple classifiers + gold standard bioboxes file
   - Accepts either taxpasta standardized TSV files OR raw profiler outputs (.kreport, .report, .profile, etc.)
2. **Standardization** (optional): If raw profiler outputs are provided, automatically standardize them using taxpasta
3. **Conversion**: Transform taxpasta format to CAMI Bioboxes format
4. **Evaluation**: Run OPAL to compute performance metrics
5. **Reporting**: Generate HTML reports with MultiQC

## Samplesheet input

You will need to create a samplesheet with information about the taxonomic profiles you would like to benchmark before running the pipeline. Use the `--input` parameter to specify its location. It must be a comma-separated file with 4 columns and a header row.
...
The pipeline supports two types of input files:

#### 1. Taxpasta standardized TSV format (recommended)

Taxpasta files should be TSV format with at minimum these columns:
- `taxonomy_id`: NCBI taxonomy ID
- `count`: Read count for this taxon

For more information on generating taxpasta files, see the [taxpasta documentation](https://taxpasta.readthedocs.io/) or run [nf-core/taxprofiler](https://nf-co.re/taxprofiler).
...
- Files will be automatically standardized using taxpasta before evaluation
- Use `--save_standardised_profiles` to save the standardized TSV files
...
> python3 bin/validate_taxpasta.py /path/to/sample_kraken2.tsv
>
> # Validate gold standard
> python3 bin/validate_bioboxes.py gold_standard.bioboxes
> ```
>
> See [Local Testing & Validation](local_testing.md#validation-scripts) for more details.

### Gold standard format

The gold standard file must be in CAMI profiling Bioboxes format. Example:

```
@SampleID:benchmark_sample
@Version:0.9.1
@Ranks:superkingdom|phylum|class|order|family|genus|species
@TaxonomyID:NCBI
@@TAXID	RANK	TAXPATH	TAXPATHSN	PERCENTAGE
562	species	131567|2|1224|1236|135622|543|561|562	Biota|Bacteria|...	0.45
1280	species	131567|2|1239|90964|1385|90964|1279|1280	Biota|Bacteria|...	0.35
```

For more information on the CAMI Bioboxes format, see the [CAMI documentation](https://github.com/bioboxes/rfc/tree/master/data-format).

### Example with params file
